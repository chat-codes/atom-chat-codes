...
class ChatChannel extends EventEmitter {
    constructor(channelName, pusher) {
    	...
    	this.channel.bind('client-editor-changed', (data) => {
            var editors = atom.textEditors.editors;
            var editor;
            editors.forEach( (e) => {
                if(e.getTitle() === 'editor.component.ts') {
                    editor = e;
                }
            });
            const {oldText, newText, timestamp} = data;
            const oldRange = new Range(data.oldRange.start, data.oldRange.end);
            const newRange = new Range(data.newRange.start, data.newRange.end);
